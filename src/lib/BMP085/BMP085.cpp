...
///Barometro
BMP085::BMP085(float recalT) : Baro(0x77, recalT)
{
}
...
bool BMP085::readAll()
{
	...
	if (state)
	{
		...
		if (state)
		{
			...
			if (b7 < 0x80000000) pascal_aux = (b7 << 1) / b4;
			else pascal_aux = (b7 / b4) << 1;

			x1 = (pascal_aux >> 8) * (pascal_aux >> 8);
			x1 = (x1 * 3038) >> 16;
			x2 = (-7357 * pascal_aux) >> 16;
			pascal_aux += (x1 + x2 + 3791) >> 4;
			pascal = float(pascal_aux);
			lastWorkT = thisReadT;
		}
	}
	lastReadT = thisReadT;
